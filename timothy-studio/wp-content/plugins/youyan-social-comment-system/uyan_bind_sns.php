    ...
    <div style="clear:both;"></div>
        ...
        <span style="color: #BC0B0B;">新发布的文章将同步到SNS中，并且SNS评论信息也能及时同步到评论系统中</span>，绑定SNS后，您将在文章发布页右侧看到同步选项：
          ...
          <div class="clear"></div>
            ...
            <div style=" margin-top:-3px; margin-left:25px;float:left; color:#f00;<?php if(get_option('SINA_EXPIRES_iN') && time() > get_option('SINA_EXPIRES_iN') ){echo 'display:block;';}else{echo 'display:none;';}?>">新浪微博授权已过期</div>
          </div></div>
            </div>

          <div id="connectWrapperConnected" style="float:left; margin-top:5px;<?php if(get_option('SINA_EXPIRES_iN')){echo 'display:block;';}else{echo 'display:none;';}?>">
		  ...
		  <div class="clear"></div>
				...
				<div class="connectWrapper" id="connectWrapper" style="float: left; margin-top:5px;<?php if(get_option('SINA_EXPIRES_iN') && time() > get_option('SINA_EXPIRES_iN') ){echo 'display:block;';}else{echo 'display:none;';}?> ">
