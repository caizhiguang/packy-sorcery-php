	...
	$.fn.extend({
		...
		drag:function(options){
			...
			panels.each(function(i,n){
				/**设置把手文本为不可选择**/
				var handles = $(n).find(options.handle);
				...
				if(options.handle == ""){
					...
					$(n).find(options.handle).bind("mousedown",$(n),function(e){
						$(this).css({ cursor:"move" });
						panel = $.fn.drag.options.activePanel = e.data;//记录当前活动中的块
			...
			if(options.handle == ""){
				...
				this.find(options.handle).bind("mousedown",panels,function(e){
					$(this).css({ cursor:"move" });
