;(function($){
	...
	$.TalkCenter.classes.mvc.view.talkView = $.salvia.object.Class($.classes.mvc.view,{
				...
				binding:{
					...
					_notice:{
						sender_name:function(sender_name){
							this.dom.find(".sender").text(sender_name);
						},
						title:function(title){
							this.dom.find(".notice_title").text(title);
						},
						time:function(time){
							var time = $.stringToDate(time);
							this.dom.find(".time").text($.dateToString(time,"Y-M-D"));
						}
					},
					send:{
		...
		initControllerListener:function(){
			...
			//更新群通知
			this.addCtrlListener("requestGroupNoticeAfter",function(e,gid,notices){
				var form = e.data._data.formsHash[gid];
				e.data._data.groupNotice[gid] = [];
				form.dom.find("tbody>tr").eq(1).hide();
				form.dom.find("tbody>tr").eq(0).hide();
				if(notices.length<=0){
					form.dom.find("tbody>tr").eq(0).show();
					return ;
				}
				for(var i in notices)
				{
					var item = form.addGroupNotice(form.dom.find("tbody>tr").eq(1).clone(),notices[i],e.data.config.binding._notice);
					item.hasCheckBox(true);
					item.dom.show().find(".notice_title").bind("click",e.data,function(e){
						var item = $(this).parents("tr").data("ui-class");
						e.data.viewNotice(item._data);
						return false;
					});
					e.data._data.groupNotice[gid].push(item);
				}
			},this);
			
			this.addCtrlListener("sendReceiptAfter",function(e,data){
				var form = e.data._data.formsHash[data.gid];
				var notices = e.data._data.groupNotice[data.gid];
				for(var i in notices)
				{
					if(notices[i]._data.id == data.id){
						notices[i]._data.receipt = "0";
						break;
					}
				}
				form.dom.find(".noticeView form.receipt").hide();
			},this);
		},
		...
		createForm:function(data){
			...
			form.dom.find(".noticeView form.receipt").bind("submit",{that:this,form:form},function(e){
				var data = e.data.form.dom.find("form.receipt").serializeObject();
				e.data.that.request("sendReceipt",[data]);
				return false;
			});
			form.dom.find("table.notice .fun input[type='checkbox']").bind("click",{that:this,gid:data.id,form:form},function(e){
				var checked = this.checked;
				for(var i in e.data.that._data.groupNotice[e.data.gid]){
					var item = e.data.that._data.groupNotice[e.data.gid][i];
					item.checked(checked);
				}
				
				e.data.form.dom.find("table.notice .fun input[type='checkbox']").each(function(){
		...
		viewNotice:function(notice){
			var form = this._data.formsHash[notice.gid];
			form.viewGroupNotice(notice.id);
		},
		...
		show:function(data,online){
			...
			if(data._type=="group"){
				this.request("requestGroupMemberByGroupId",[data.id]);
				this.request("requestGroupNoticeByGid",[data.id]);
			}
		},
